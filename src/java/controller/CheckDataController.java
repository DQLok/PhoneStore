...
public class CheckDataController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            String xmlPath = request.getServletContext().getRealPath("/xml/fileXML.xml");           
            String xsdPath = request.getServletContext().getRealPath("/xml/fileXSD.xsd");
            if(XSDValidator.validateXMLSchema(xsdPath, xmlPath)){
                url = INDEX_PAGE;
                request.setAttribute("SUCCESS", "No problem");
            }else{
                url = ERROR_PAGE;
            }
        }catch(Exception e){
